...
This repository does not yet include real downstream business side effects. A future consumer group load test should keep group `order-events`, process messages idempotently by `event_id`, acknowledge after side effects commit, and route poison messages to `stream:orders:dead_letter`.
